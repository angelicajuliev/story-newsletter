...
from newsletter.models import Category, Recipient
...
class TestUnsubscribeByEmailAndCategory:
    ...
    def test_unsubscribe_by_email_and_category_not_found(
        self, create_recipient, create_category
    ):
        # Arrange
        email = "myrandomemail.com"
        category = create_category("Test Category")
        ...
        expected_categories = Recipient.objects.get(
            email=email
        ).category_subscription.count()
        ...
        assert Recipient.objects.filter(email=email).count() == 1
        assert (
            Recipient.objects.get(email=email).category_subscription.count()
            == expected_categories
        )

    def test_unsubscribe_by_email_and_category_recipient_not_found(
